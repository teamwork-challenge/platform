...
4. remaining = max(0, max_tasks_per_team - total). Если max_tasks_per_team отсутствует — remaining считаем 0.
...
- Структура коллекций:
  - /rounds/{roundId}/teams/{teamId}/dashboard/{roundTaskTypeId}
  - Поля документа Dashboard: round_id, team_id, round_task_type_id, 
    type (значение RoundTaskType.type), pending, ac, wa, remaining, updated_at
  - Уникальность документа dashboard: (round_id, team_id, round_task_type_id).
- Документ создаётся, когда команда берёт новую задачу данного типа в раунде.
