...
5. Сортировка:
   ...
   - если и это совпадает — по имени команды по возрастанию (детерминированный порядок).
...
Реализация (Firebase / Firestore):
- Структура коллекций:
  - /rounds/{roundId}/leaderboard/{teamId}
  - Поля документа LeaderboardTeam: round_id, team_id, team_name,
    scores (map<string round_task_type, int score>), total_score (int),
    last_score_at (timestamp для тай‑брейка), updated_at (timestamp).
  - Уникальность документа leaderboard: (round_id, team_id).
- Правила сортировки — как описано выше.

Переходы состояний (влияние на рейтинг):
- Сабмит решения:
  - PENDING → AC: total_score += score; scores[type] += score; last_score_at = now(); updated_at = now()
  - PENDING → WA: рейтинг не меняется
  - WA → AC: total_score += score; scores[type] += score; last_score_at = now(); updated_at = now()
